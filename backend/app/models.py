...
class Portfolio(SQLModel, table=True):
    ...
    projects: Optional[List[Dict]] = Field(
        default=None, sa_column=Column(JSON), description="Список проектов с описаниями"
    )

    # Опыт работы
    work_experience: Optional[List[Dict]] = Field(
        default=None, sa_column=Column(JSON), description="История работы"
    )
    ...
    contact_info: Optional[Dict] = Field(
        default=None, sa_column=Column(JSON), description="Email, телефон, соцсети"
    )

    # Настройки внешнего вида
    template_id: Optional[str] = Field(default="default", max_length=50)
    theme: Optional[str] = Field(default="light", max_length=20)
...
class GenerationHistory(SQLModel, table=True):
    ...
    generation_type: str = Field(
        max_length=50, description="about, project, skills, full"
    )
    ...
    # Статус
    status: str = Field(
        default="success", max_length=20, description="success, failed, cached"
    )
